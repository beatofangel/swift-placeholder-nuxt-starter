<template>
  <v-menu v-model="showPicker" :close-on-content-click="true" offset-y>
    <template v-slot:activator="{ on, attrs }">
      <v-text-field
        :value.sync="icon"
        :prepend-inner-icon="`mdi-${icon && icons.includes(`mdi-${icon}`) ? icon : 'blank'}`"
        :prefix="prefix"
        :label="label"
        :placeholder="placeholder"
        @input="scrollToSearch"
        @change="validateIcon"
    ...
    <v-card v-intersect="onIntersect">
      ...
      >
        ...
        <v-list-item
          :key="index"
          @click="icon=item.substring(4)"
          :input-value="icon==item.substring(4)"
...
export default {
  ...
  methods: {
    scrollToSearch(val) {
      ...
      const icon = `mdi-${val}`
      let idx = this.icons.findIndex(i=>i==icon)
      if (idx != -1) {
        this.$emit('input', val)
      }
      idx == -1 && (idx = this.icons.findIndex(i=>i.startsWith(icon)))
      idx == -1 && (idx = this.icons.findIndex(i=>i.includes(val)))
      if (idx != -1) {
        // setTimeout(() => {
          this.$vuetify.goTo( idx * 48 + 48, { container: '.iconOptions', easing: 'easeInOutCubic' })
        // }, 500);
      }
    },
    onIntersect(entries/*, observer*/) {
      entries[0].isIntersecting && !!this.icon && this.scrollToSearch(this.icon)
    },
    validateIcon(val) {
      const icon = `mdi-${val}`
      let idx = this.icons.findIndex(i=>i==icon)
