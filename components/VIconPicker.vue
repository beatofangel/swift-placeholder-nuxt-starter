<template>
  <v-menu v-model="showPicker" :close-on-content-click="true" location="bottom" :offset="[45, 0]">
    <template v-slot:activator="{ props }">
      <v-text-field v-model="icon" :prepend-inner-icon="`mdi-${icon && icons.includes(`mdi-${icon}`) ? icon : 'blank'}`"
        :prefix="prefix" :label="label" :placeholder="placeholder" @update:model-value="scrollToSearch"
        @change="() => validateIcon(icon)" :error-messages="errorMessages" v-bind="props" clearable
        :persistent-placeholder="persistentPlaceholder">
      </v-text-field>
    </template>
    <v-card v-intersect="onIntersect">
      <v-virtual-scroll ref="iconOptions" class="iconOptions" bench="5" :items="icons" height="300">
        <template v-slot:default="{ item, index }">
          <v-list-item :key="index" @click="icon = item.substring(4)" active-class="primary--text">
...
const icon = computed({
  ...
  set(val) {
    console.log('set icon', val)
    emits('update:modelValue', val!)
  }
})
const iconOptions = ref()

const scrollToSearch = (val: string) => {
  const tempIcon = `mdi-${val || ''}`
  if (!val) {
    // this.$emit('update:modelValue', val)
    icon.value = val || ''
    console.log('iconpicker empty value', val)
    return
  }
  showPicker.value = true
  let idx = icons.value.findIndex(i => i == tempIcon)
  if (idx != -1) {
    icon.value = val
    // this.$emit('update:modelValue', val)
  }
  idx == -1 && (idx = icons.value.findIndex(i => i.startsWith(tempIcon)))
  idx == -1 && (idx = icons.value.findIndex(i => i.includes(val)))
  if (idx != -1) {
    iconOptions.value && iconOptions.value.scrollToIndex(idx)
  }
}
const onIntersect = (_: boolean, entries: Array<IntersectionObserverEntry>/*, observer*/) => {
  entries[0].isIntersecting && !!icon.value && scrollToSearch(icon.value)
}
const validateIcon = (val?: string) => {
  const tempIcon = `mdi-${val || ''}`
  let idx = icons.value.findIndex(i => i == tempIcon)
  if (idx == -1) {
    setTimeout(() => {
